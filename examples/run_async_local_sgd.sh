...
RESULT_DIR=$HALOS_DATA_DIR/async_local_sgd_results

# Async-Local-SGD hyperparms
GLR=0.05
GBETA=0.9
GMUD=32
N_LOCAL_STEPS=32
...
mkdir -p $RESULT_DIR
python -m halos.simulation.exec_async_local_sgd \
  --exp_name halos/async_local_sgd_results \
  ...
  --ps_opt_config "{\"opt_type\": \"delayed_nesterov\", \"lr\": $_GLR, \"beta\": $GBETA, \"buffer_size\": $GMUD }" \
